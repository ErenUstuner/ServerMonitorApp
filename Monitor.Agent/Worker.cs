...
using System.IO;
using System.Linq;
using System.Net; // YENİ: IP Eşleştirmesi için
...
namespace Monitor.Agent
{
    public class Worker : BackgroundService
    {
        ...
        {
            _logger = logger; _httpClient = new HttpClient();
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                try { await _httpClient.PostAsJsonAsync(_apiUrl, payload, stoppingToken); } catch { }
...
#pragma warning restore CA1416
            ...
            catch { return (serviceName, "Bulunamadı", false); }
        }
        ...
        private void RestartWindowsService(string serviceName)
        {
            try
            {
                WriteLog($"[AJAN] '{serviceName}' restart emri alındı.");
#pragma warning disable CA1416
...
#pragma warning restore CA1416
            ...
            catch (Exception ex) { WriteLog($"[AJAN HATA] Restart başarısız: {ex.Message}", "ERROR"); }
        }
        ...
        private void WriteLog(string message, string level = "INFO")
        {
            try
            {
                string logDir = @"C:\ProgramData\MonitorServis\Log"; Directory.CreateDirectory(logDir);
                File.AppendAllText(Path.Combine(logDir, "agent.log"), $"[{DateTime.Now:yyyy-MM-dd HH:mm:ss.fff}] [{level}] {message}{Environment.NewLine}");
                if (level == "ERROR") _logger.LogError(message); else _logger.LogInformation(message);
            }
            catch { }
        }
    }
